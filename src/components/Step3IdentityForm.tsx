...
const ROLES: { id: RoleType; name: string; icon: React.ReactNode; desc: string }[] = [
  { id: 'street_racer', name: 'Street Racer', icon: <Car className="w-5 h-5 text-vice-cyan" />, desc: 'Dominates Midnight Drag & Circuit Races' },
  { id: 'hustler', name: 'Vice Hustler', icon: <Zap className="w-5 h-5 text-vice-pink" />, desc: 'Street Smarts & Underground Deals' },
  { id: 'entrepreneur', name: 'Syndicate Mogul', icon: <Briefcase className="w-5 h-5 text-vice-gold" />, desc: 'Front Businesses & High Society Wealth' },
  { id: 'hacker', name: 'Cyber Ghost', icon: <Laptop className="w-5 h-5 text-purple-400" />, desc: 'Bank Security Bypass & Data Heists' },
  { id: 'biker', name: 'Outlaw Biker', icon: <Bike className="w-5 h-5 text-vice-orange" />, desc: 'Enforcer & Highway Smuggling Crew' },
];

const CREWS: { id: CrewType; name: string; desc: string }[] = [
  { id: 'solo', name: 'Solo Lone Wolf', desc: 'No alliances. 100% cut of profits.' },
  { id: 'night_riders', name: 'Night Riders Crew', desc: 'High-speed getaways & street racing dominance.' },
...
export const Step3IdentityForm: React.FC<Step3IdentityFormProps> = ({
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-8">
      ...
      {/* Main Form Container */}
      <div className="vice-glass p-6 sm:p-8 rounded-3xl border border-vice-pink/30 space-y-8">
        
        {/* Name & Alias */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="text-xs font-bold font-orbitron text-white uppercase tracking-wider flex items-center gap-1.5">
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-xl bg-[#090714] border border-white/15 focus:border-vice-pink text-white font-bold text-sm focus:outline-none focus:ring-2 focus:ring-vice-pink/30 transition-all"
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-xl bg-[#090714] border border-vice-pink/50 text-vice-pink font-extrabold text-sm focus:outline-none focus:ring-2 focus:ring-vice-pink/50 transition-all"
            />
          </div>
        </div>
              ...
              return (
                <button
                  ...
                  className={`p-4 rounded-2xl border text-left flex flex-col justify-between transition-all ${
                    isSelected
                      ? 'bg-vice-cyan/20 border-vice-cyan text-white shadow-[0_0_20px_rgba(0,240,255,0.4)] scale-105'
              ...
              return (
                <button
                  ...
                  className={`p-4 rounded-2xl border text-left transition-all ${
                    isSelected
                      ? 'bg-vice-pink/20 border-vice-pink text-white shadow-[0_0_15px_rgba(255,0,127,0.4)]'
                      : 'bg-white/5 border-white/10 text-slate-400 hover:bg-white/10'
                  }`}
                >
          ...
          {profile.crew === 'syndicate' && (
            <div className="pt-2">
              <input
                ...
                className="w-full px-4 py-2.5 rounded-xl bg-[#090714] border border-vice-pink text-white font-semibold text-xs focus:outline-none"
              />
            </div>
          )}
        </div>
        ...
        {/* Motto Input */}
        <div className="space-y-2">
          <label className="text-xs font-bold font-orbitron text-white uppercase tracking-wider">
          ...
          <input
            ...
            className="w-full px-4 py-3 rounded-xl bg-[#090714] border border-white/15 text-white font-medium text-xs focus:outline-none focus:border-vice-cyan"
          />
        </div>
